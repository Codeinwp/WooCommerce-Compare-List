...
function wccm_add_button() {
	...
	if ( in_array( $product_id, wccm_get_compare_list() ) ) {
		...
		echo '<a href="wccm-button', esc_url( $url ), '" class="', implode( ' ', $classes ), '">', esc_html( $text ), '</a>';
		if ( is_single() ) {
			echo '<a href="', wccm_get_compare_page_link( wccm_get_compare_list() ), '" class="wccm-button button alt">', __( 'View compare', 'wccm' ), '</a>';
		}
	} else {
